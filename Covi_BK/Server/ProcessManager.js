        ...
        if (message.type === 'utf8') {// accept only texts
            ...
            if (json.type === 'sendCode') {
				...
				excute.stderr.on('data', function(data) {
					console.log(excute.pid + " stderr : " +data);
				});
				
				excute.on('exit', function(data) {
					console.log("Exit Process : " + excute.pid);
				});
				...
				if (excute == null || excute == undefined) {
					console.log('reakpoint : Input Console\n');
				} else {
				...
				if (excute == null || excute == undefined) {
					console.log('clearbreakpoint : Input Console\n');
				} else {
					var tempCmd = coviRoot + json.data.substring(2);
				...
				console.log('One user enter the CoVi, User Name : ' + json.userName + '\n');
