...
def main(pidNum, userID, Projectfile):
    ...
    if CorPython.count('.py'):
        ...
        makefile = Projectfile[:-2]
        #Projectfile = "/home/junbum/workspace/CodeVisualizerDebugger/src/ExampleCode/c/example.c"
        
        subprocess.Popen(['gcc','-o',makefile,'-g', Projectfile], stdin=subprocess.PIPE, stdout=subprocess.PIPE)
        time.sleep(0.5)
        Debugger.start(['gdb', '-i', 'mi', '-q'])
        Debugger.openFile(makefile)
