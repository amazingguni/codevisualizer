...
function drawSpecialVarGroup(name, target) {
	...
	var specialDraw = drawSpecialVar(name, tmpType, tmpValue, tmpIndex);
	specialDraw.group.setName("specialvar");

	specialChildArr = tmpValue;

	return {
		group : specialDraw.group,
		len : specialDraw.len
	};
}
...
function drawSpecialVar(name, type, dataArr, dataIndex) {
	...
	for (var i = 0; i < dataArr.length; i++) {
		...
		if (preGroup != null) {
			if (i % 5 == 0) {
				startYCount++;
				startXCount = -1;
				startY = specialGroup[i].getY() + (200*startYCount);
				startX = specialGroup[i].getX();
			}
			startXCount++;
			specialGroup[i].setY(startY);
			specialGroup[i].setX(startX + (startXCount*tmpType.getWidth()));
		}
	...
	tmpName.setWidth(specialGroup[i - 1].getX() + tmpType.getWidth());
	tmpType.setWidth(specialGroup[i - 1].getX() + tmpType.getWidth());
	tmpName.setY(-23);
	tmpType.setY(preGroup.getY() + preGroup.children[0].getHeight() + 10);
