$(function () {
    ...
    // open connection
    var connection = new WebSocket('ws://211.189.127.97:1337');
    ...
    connection.onmessage = function (message) {
        ...
        if (json.type === 'message') { // it's a single message
            ...
            input_fileName.removeAttr('disabled');
            input_console.removeAttr('disabled');
        	 loadCodeFile(json.data.userName, json.data.text, new Date(json.data.time));
        	 myUserIdx = json.data.userIdx;
        } else {
    ...
    */
    ...
    input_console.keydown(function(e) {
        if (e.keyCode === 13) {
            ...
            // sends back response
            input_console.attr('disabled', 'disabled');
            addMessage('','Input : ' + input_console.val(),'');
        }
    });
    ...
    function loadCodeFile(userName, message, dt) {
        input_code.html(message);
        // content_output.html(message);
        // console.log(message);
        //content_output.html(message.split("\n").join("</br>"));
        content_output.html("<pre>" + message.replace(/\n/gi, "</br>") + "</pre>");
    ...
    window.onload = function() {
       ...
       document.getElementById("exit").onclick = function() {
            var json = JSON.stringify({ type:'exit', 
        	 								userName: myUserName, 
            								userIdx: myUserIdx });
            //window.alert(json);
            connection.send(json);
       ...
       document.getElementById("loadFile").onclick = function() {
            var json = JSON.stringify({ type:'loadFile', 
        	 								userName: myUserName, 
        	 								fileName: input_fileName.val(), 
            								userIdx: myUserIdx });
            //window.alert(json);
       ...
       document.getElementById("reName").onclick = function() {
            var json = JSON.stringify({ type:'reName', 
        	 								userName: myUserName, 
        	 								fileName: input_fileName.val(), 
        	 								reName:input_fileName.val()+"_rename",
            								userIdx: myUserIdx });
            //window.alert(json);
            connection.send(json);
            input_code.attr('disabled', 'disabled');
