...
wsServer.on('request', function(request) {
    ...
    connection.on('message', function(message) {
    	console.log((new Date()) + ' Message type ' + message.type);
        if (message.type === 'utf8') { // accept only texts
        	...
        	var excute = spawn("mkdir", ["../"+message.utf8Data]);
        	excute.stdout.addListener("data", function(data) {
                  cliData = data.toString();
                  message.utf8Data = cliData.substr(0, cliData.length-1);
                  
              	console.log((new Date()) + ' make folder '
                            + ': ' + message.utf8Data
                            +' len: '+cliData.length);
                                    // we want to keep history of all sent messages
                var obj = {
        ...
        } else if(message.type === 'makeFile'){
        	var excute = spawn("mkdir", ["../"+message.utf8Data]);
        	excute.stdout.addListener("data", function(data) {
                  cliData = data.toString();
                  message.utf8Data = cliData.substr(0, cliData.length-1);
                  
              	console.log((new Date()) + ' make file '
                            + ': ' + message.utf8Data
                            +' len: '+cliData.length);
                                    // we want to keep history of all sent messages
                var obj = {
